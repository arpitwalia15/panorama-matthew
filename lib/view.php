...
function psppan_essentials($id, $style = null, $docs = null)
{
	...
	$psppan_essentials = '<div id="psp-essentials" class="' . $style . '">
	...
	if (($startDate) || ($endDate)):
		...
		$psppan_essentials .= '<h4>' . __('Project Timing', 'psp_projects') . '</h4>';
		...
		$psppan_essentials .= '<ul class="timing">';
	...
	if ($docs != 'none'):
		...
		$psppan_essentials .= '<div id="project-documents" class="' . $style . '">
		...
		if (PSP_PLUGIN_TYPE == 'professional') {
			...
			if (get_field('documents', $id)) {
				...
				$psppan_essentials .= psppan_documents($id);
			} else {

				$psppan_essentials .= '<p>' . __("No documents at this time.", 'psp_projects') . '</p>';
			}
		} else {
			...
			if (!empty($documents_text)) {
				...
				$psppan_essentials .= $documents_text;
			} else {
	...
	$psppan_essentials .= '</div> <!--/#psp-essentials--> ';



	return $psppan_essentials;
}
...
function psppan_phases($id, $style = null, $taskStyle = null)
{
	...
	if ($style == 'psp-shortcode') {
		...
		$psppan_phases = '<div class="psp-shortcode-phases">';
	} else {

		$psppan_phases = '<div class="psp-row">';
	}
	...
	$i = 0;
	$c = 0;
	while (has_sub_field('phases', $id)): $i++;
		$c++;



		if (($i % 2 == 1) && ($style != 'psp-shortcode')) {
			$psppan_phases .= '</div><div class="psp-row">';
		}
		...
		$psppan_phases .= '<div class="psp-phase color-' . $color . '">';
		...
		$psppan_phases .= '<h3><span class="psp-top-complete">' . $completed . '% <b>' . __('Complete', 'psp_projects') . '</b></span> ' . get_sub_field('title') . '</h3>
		...
		if ($completed != 0) {
			...
			$psppan_phases .= '<li data-value="' . $completed . '"><span></span> ' . $completed . '% ' . __('Completed', 'psp_projects') . '</li>';
		} else {

			$psppan_phases .= '<li data-value="' . $completed . '" class="completed-datapoint"><span></span> ' . $completed . '% ' . __('Completed', 'psp_projects') . '</li>';
		}

		if (($remaining != 100) && ($remaining != 0)) {

			$psppan_phases .= '<li data-value="' . $remaining . '"><span></span>' . $remaining . '% ' . __('Remaining', 'psp_projects') . '</li>';
		}
		...
		if ((get_sub_field('tasks', $id)) && ($taskStyle != 'no')):
			...
			$psppan_phases .= '<h4><a href="#" class="task-list-toggle">' . $taskbar . __('Tasks', 'psp_projects') . '</a></h4>
...
function panorama_login_form()
{
	...
	// Generate the login form with sanitized redirect URL
	$panorama_form = wp_login_form(array('redirect' => $redirect_url));
...
function psppan_documents($post_id)
{
...
function psppan_dashboard_overview_widget_function()
{
	...
	<ul class="psp-projects-overview">
		...
		<li><span class="psp-dw-projects"><?php echo esc_html($total_projects); ?></span> <strong><?php echo esc_html_e('Projects', 'psp_projects'); ?></strong> </li>

		<li><span class="psp-dw-completed"><?php echo esc_html($completed_projects); ?></span> <strong><?php echo esc_html_e('Completed', 'psp_projects'); ?></strong></li>

		<li><span class="psp-dw-active"><?php echo esc_html($active); ?></span> <strong><?php echo esc_html_e('active', 'psp_projects'); ?></strong></li>

		<li><span class="psp-dw-types"><?php echo esc_html($taxonomies); ?></span> <strong><?php echo esc_html_e('Types', 'psp_projects'); ?></strong></li>
	...
	<h4><?php echo esc_html_e('Recently Updated', 'psp_projects'); ?></h4>

	<table class="psp-dashboard-widget-table">

		<tr>

			<th><?php echo esc_html_e('Project', 'psp_projects'); ?></th>

			<th><?php echo esc_html_e('Progress', 'psp_projects'); ?></th>

			<th><?php echo esc_html_e('Updated', 'psp_projects'); ?></th>
	...
	while ($recent->have_posts()): $recent->the_post();
		...
		echo '<tr><td><a href="' . esc_url(get_edit_post_link()) . '">' . esc_html(get_the_title()) . '</td><td>';
		...
		if ($completed > 10) {
			...
			echo '<p class="progress"><span class="psp-' . esc_attr($completed) . '"><strong>%' . esc_html($completed) . '</strong></span></p>';
		} else {

			echo '<p class="progress"><span class="psp-' . esc_attr($completed) . '"></span></p>';
		}

		echo '</td><td class="psp-dwt-date">' . esc_html(get_the_modified_date("m/d/Y")) . '</td><td class="psp-dwt-date"><a href="' . esc_url(get_permalink()) . '" target="_new" class="psp-dw-view">' . esc_html_e('View', 'psp_projects') . '</a></td>
