...
abstract class CMB2_Type_Counter_Base extends CMB2_Type_Base {
	...
	public function char_counter_markup( $val ) {
		...
		switch ( $type ) {
			case 'words':
				$label = $char_max
					? $this->_text( 'words_left_text', esc_html__( 'Words left', 'psp_projects' ) )
					: $this->_text( 'words_text', esc_html__( 'Words', 'psp_projects' ) );
				break;
			default:
				$type  = 'characters';
				$label = $char_max
					? $this->_text( 'characters_left_text', esc_html__( 'Characters left', 'psp_projects' ) )
					: $this->_text( 'characters_text', esc_html__( 'Characters', 'psp_projects' ) );
				break;
		}

		$msg = $char_max
			? sprintf( '<span class="cmb2-char-max-msg">%s</span>', $this->_text( 'characters_truncated_text', esc_html__( 'Your text may be truncated.', 'psp_projects' ) ) )
			: '';
