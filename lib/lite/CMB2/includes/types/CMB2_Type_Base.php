...
abstract class CMB2_Type_Base {
	...
	public function __call( $method, $arguments ) {
		...
		switch ( $method ) {
			...
			default:
                // translators: %1$s, %2$s represents the field key that caused the error.
				throw new Exception( sprintf( esc_html__( 'Invalid %1$s method: %2$s', 'psp_projects' ), __CLASS__, esc_html($method) ) );
	...
	public function __get( $field ) {
		...
		switch ( $field ) {
			...
			default:
                // translators: %1$s, %2$s represents the field key that caused the error.
				throw new Exception( sprintf( esc_html__( 'Invalid %1$s property: %2$s', 'psp_projects' ), __CLASS__, esc_html($field) ) );
