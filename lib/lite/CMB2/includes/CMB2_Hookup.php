...
class CMB2_Hookup extends CMB2_Hookup_Base
{
	...
	public function term_hooks()
	{
		if (! function_exists('get_term_meta')) {
			wp_die(esc_html__('Term Metadata is a WordPress 4.4+ feature. Please upgrade your WordPress install.', 'psp_projects'));
		}

		if (! $this->cmb->prop('taxonomies')) {
			wp_die(esc_html__('Term metaboxes configuration requires a "taxonomies" parameter.', 'psp_projects'));
		}
	...
	public function context_box_title_markup_open($add_handle = true)
	{
		...
		$toggle_button = sprintf(
			...
			sprintf(__('Toggle panel: %s', 'psp_projects'), $title)
		);
