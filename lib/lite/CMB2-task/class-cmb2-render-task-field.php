...
function cmb2_render_callback_for_tasks($field, $value, $object_id, $object_type, $field_type)
{
	...
	$value['complete'] = sanitize_text_field($value['complete']);
?>
	<div>
		<p><label for="<?php echo esc_attr($field_type->_id('_title')); ?>"><?php esc_html_e('Task', 'psp_projects'); ?></label></p>
		<?php
		echo $field_type->input(array(
			...
			'value' => $value['title'],
			'desc'  => '',
		)); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- only safe HTML output generated by this function. 
		?>
	</div>
	<div>
		<p><label for="<?php echo esc_attr($field_type->_id('_complete')); ?>"><?php esc_html_e('Completion', 'psp_projects'); ?></label></p>
		<?php
		echo $field_type->select(array(
			'name'    => $field_type->_name('[complete]'),
			'id'      => $field_type->_id('_complete'),
			'options' => psppan_lite_completion_options($value['complete']),
	...
	<br class="clear">
	<?php
	echo $field_type->_desc(true); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- only safe HTML output generated by this function.
}

add_filter('cmb2_render_tasks', 'cmb2_render_callback_for_tasks', 10, 5);

function psppan_lite_completion_options($value = false)
{
...
class CMB2_Render_Task_Field extends CMB2_Type_Base
{
	...
	public function render()
	{
		...
		foreach ($complete_list as $val => $label) {
			$complete_options .= '<option value="' . esc_attr($val) . '" ' . selected($value['complete'], $val, false) . '>' . esc_html($label) . '</option>';
		}

		ob_start();
	?>
		<div>
			<p><label for="<?php echo esc_attr($this->_id('_title', false)); ?>"><?php echo esc_html($this->_text('task_title_text', 'Task')); ?></label></p>
			<?php
		...
		<div>
			<p><label for="<?php echo esc_attr($this->_id('_complete', false)); ?>"><?php echo esc_html($this->_text('task_completion_text', 'Completion')); ?></label></p>
			<select
				name="<?php echo esc_attr($this->_name('[complete]')); ?>"
				id="<?php echo esc_attr($this->_id('_complete')); ?>">
				<?php echo $complete_options; ?>
			</select>
		</div>
		<p class="clear">
			<?php
			echo $this->_desc();  //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- only safe HTML output generated by this function. 
			?>
		</p>
<?php
