...
<div class="<?php echo esc_attr($wrapper_class); ?> cf psp-total-phases-<?php echo esc_attr(psp_get_phase_count()); ?>">
	...
	if( !empty( $phases ) ):
		...
		<div data-phase-index="<?php echo esc_attr($i); ?>" class="psp-phase color-<?php echo esc_attr( $color ); ?> psp-phase-progress-<?php echo esc_attr( $completed ); ?>" id="phase-<?php echo esc_attr($i); ?>">
			<h3>

				<?php echo esc_html( !empty($phase['title']) ? $phase['title'] : '' ); ?>

				<span class="psp-top-complete">
					<span class="psp-top-meta">
						<?php echo esc_html($completed) . '% ' . esc_html_e( 'Complete', 'psp_projects' ); ?>
					</span>
				...
				<div class="psp-chart">
					...
					<script>
						document.addEventListener("DOMContentLoaded", function() {
						    ...
						    var chartCanvas = document.getElementById("chart-<?php echo esc_attr($i); ?>");
						    if (chartCanvas) {
						        var chartContext = chartCanvas.getContext("2d");
						        allCharts[<?php echo esc_attr($i); ?>] = new Chart(chartContext).Doughnut(data, chartOptions);
						    }
						});
					</script>
